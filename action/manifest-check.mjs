...
export function reconcileEntries(manifest, discoveredFiles) {
  ...
  for (const entry of entrySet) {
    if (!fileSet.has(entry)) {
      errors.push(`${entry} is listed in ${MANIFEST_FILENAME} entries but was not found on disk`);
    }
  }

  for (const file of fileSet) {
    if (!entrySet.has(file)) {
      errors.push(`${file} exists but is not listed in ${MANIFEST_FILENAME} entries`);
    }
  }
